...
- `lib/dish-image.ts` — source-photo download/storage for URL imports; it does not generate images.
- `lib/image-model.ts` + `lib/dish-image-job.ts` — the single generated-image policy and durable job lifecycle. **Every Dinner-generated dish image explicitly uses Nex Image Generator's Codex Provider model `gpt-image-2`**, submitted through claude-agent's provider-neutral `/api/v1/images/batch` contract (including one-image jobs). There is no premium/standard model split and no direct Gemini/Replicate call or provider fallback in Dinner Spinner. `lib/nex-image.ts` + `lib/nex-image-batch.ts` implement the Nex transport; `NEX_API_TOKEN` needs the `images:generate` scope.
...
| `NEX_API_TOKEN` | Nex API token with `chat` and `images:generate` scopes; powers recipe ingest and explicit `gpt-image-2` dish-image jobs. |
...
| `CRON_SECRET` | **Optional but recommended.** Bearer that authenticates batch-import and dish-image background completion (`/api/import/advance-bg`, `/api/dishes/images/advance-bg`) plus the daily `vercel.json` safety sweep. Unset ⇒ browser polling still advances work, but it may pause after the user closes the tab. `openssl rand -hex 32`. |
...
- **`optional` is about the INGREDIENT, not its amount or serving role.** Only set `optional: true` when the source itself makes it optional — "optional", "(optional)", "if desired", "naar keuze", or clearly suggested toppings. A required instruction such as "Serve with rice, crème fraîche, and grated cheese" does not make those ingredients optional; neither does placing them under a "To serve" heading. A *flexible quantity* ("to taste", "or less, to taste", "depending on how hot you like it") tunes the amount, NOT whether to include it — a core ingredient (garlic, lemon, agave, curry paste, nutmeg) must never be `optional` just because its quantity is to-taste. (Over-flagging silently drops it from the shopping list.)
...
2. **`POST /api/ingest`** (`app/api/ingest/route.ts`): auths the user; if the input is a bare URL, scrapes it server-side first (see **URL imports** below); builds the prompt via `lib/ingest/prompt.ts::buildIngestPrompt`, calls claude-agent's `POST /api/v1/chat-async` (`lib/ingest/nex-agent.ts::startNexAgentJob`) with an explicitly OpenAI-pinned model (`codex:gpt-6-astra` for recipe photos, `codex:gpt-5.6-sol` for text) and the `DISH_INPUT_JSON_SCHEMA`. claude-agent returns `{job_id}` in <1s. Route returns `{jobId, sourceImageUrl}` (HTTP 202) to the browser.
3. **Browser polls** `GET /api/ingest/jobs/[id]` every 1.5s for up to 3 min.
4. **`GET /api/ingest/jobs/[id]`** (`app/api/ingest/jobs/[id]/route.ts`): proxies claude-agent's `GET /api/v1/jobs/{id}` (`pollNexAgentJob`). When status flips to `done`, re-validates the `structured` payload against `DishInputSchema` (defense in depth — claude-agent enforces JSON Schema structurally but not all our semantic constraints) and returns `{status: "done", dish}`. On `failed`, returns the error envelope.
...
**Models: OpenAI only.** `lib/ingest/nex-agent.ts::RECIPE_MODELS` selects **GPT-5.6 Sol** (`codex:gpt-5.6-sol`) for text/URL parsing, batch detection/parsing, and maintenance backfills; **GPT-6 Astra** (`codex:gpt-6-astra`) reads recipe photos. These explicit prefixes override Nex's global provider setting. The client requires a supported selection and rejects missing, legacy, or non-OpenAI models before sending a request. Dish-image generation separately uses `gpt-image-2`. There is no Anthropic fallback.

The transport still calls the existing service named `claude-agent`; that service name and the compatibility environment variable `CLAUDE_AGENT_URL` do **not** select Anthropic. Nex runs these requests through its Codex structured-output path with native image attachments and strict JSON output. Prompts request JSON directly, not a `submit_result` tool call. The existing anyOf-free schema remains necessary for Nex's validation adapter.

**Import fidelity**: the prompt includes the canonical ingredient vocabulary, distinguishes Dutch `theelepel/tl` (`tsp`) from `eetlepel/el` (`tbsp`) and onion powder from garlic powder, preserves product forms, and retains method-only ingredients and optional serving suggestions. The chili source and historical failing output in `lib/ingest/fixtures/` support deterministic fidelity checks plus the opt-in live `scripts/eval-recipe-ingest.ts` evaluation. These checks evaluate that fixture; they are not a general runtime source-comparison validator.
...
- **Detect** (`POST /api/import`): claude-agent splits the document into `{title, text}` chunks via explicitly pinned **GPT-5.6 Sol** (`codex:gpt-5.6-sol`; `buildDetectPrompt`, `DETECT_JSON_SCHEMA`); chunk text is copied verbatim so the per-chunk parse reuses the single-ingest contract.
- **Parse + create**: each chunk → explicitly pinned GPT-5.6 Sol parse (`codex:gpt-5.6-sol`; `DISH_INPUT_JSON_SCHEMA`) → `normalizeEscapedWhitespace` → `createDishForUser(…, {autoImage:false})` (which assigns ingredient ids + rewrites the method's inline `#index` references to ids). Up to `PARSE_CONCURRENCY` (3) parse jobs in flight.
- **Imaging**: every user and every import size uses Nex's durable `/images/batch` API with the explicit raw model id `gpt-image-2`. Requests are split into groups of 12 so completed image bytes can be fetched, compressed, uploaded to Blob, and applied inside Vercel's 60-second budget. Legacy in-flight batches without that model marker are canceled and their still-pending dishes are resubmitted to GPT Image 2.
- **Background completion** (`CRON_SECRET`): on confirm — and again on resume — the server kicks `/api/import/advance-bg`, a `CRON_SECRET`-gated self-driving chain (does ~45s of work in `after()`, then hands off to a fresh invocation; capped at `MAX_HOPS`=60; stops on any terminal status) that advances the import to completion **even after the user closes the tab**. Browser polling still drives the live UI; the row's `locked_until` lock serializes the two so they never double-advance. A **daily** `vercel.json` cron (`GET /api/import/advance-bg`) sweeps stale non-terminal imports and pending dish-image jobs as a backstop — Hobby allows only daily crons, so the self-chain is the real-time mechanism. Unset `CRON_SECRET` ⇒ imports and image jobs advance while their UI is polling but may pause after the user closes the tab. The routes are reachable past the NextAuth proxy via its existing Bearer-token bypass.
...
- **Dish-photo generation is durable and async.** `POST /api/dishes/[id]/image` creates an `image_jobs` row and immediately submits a one-request Nex batch with `model: "gpt-image-2"`, returning `202 {jobId}` without waiting for generation. The edit page polls `GET /api/dishes/[id]/image/jobs/[jobId]`; a `CRON_SECRET`-protected self-chain polls independently and the existing daily import cron sweeps stale image jobs. `locked_until` prevents browser/background double-apply, and a newer reroll supersedes (and best-effort cancels) an older pending job. Dish creation uses this same job path; authenticated `GET /api/dishes/[id]` polling advances it as a fallback when `CRON_SECRET` is unset. `image_jobs` rows are pruned after one day on regenerate POST.
...
into `.claude/worktrees/v2-redesign`, which accounts for most of its 291 errors;
the active app also has four errors across `demo-library.tsx`,
`batch-import.tsx`, and `profile-view.tsx`. Do not
attribute that baseline to an unrelated change without checking paths.

The direct test command currently discovers 288 tests: 282 pass and six are a
