...
- `lib/dish-image.ts` — `generateAndStoreImage(dish, userId)`: prompt → provider → blob → update dish. Shared by create-route auto-gen and the async regenerate job. **Premium-model gating**: Nano Banana Pro (~$0.139/img) is reserved for the seed owner + `PREMIUM_IMAGE_EMAILS`; everyone else gets flux (~$0.04/img) on every path (single add, regenerate, bulk backfill, and batch import — non-premium imports route to the flux sync path, not the Gemini batch). Gate is `lib/auth-helpers.ts::isPremiumImageUser`; `getProvider({ premium })` drops both NBP paths when false.
