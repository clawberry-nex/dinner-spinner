...
import { Chip, DishArt } from "@/app/_components/ui";
...
export function DishBrowser({ initialDishes, allTags, isOwner }: Props) {
  ...
  return (
    <>
      <div className="mb-3 text-[11px] uppercase tracking-[0.12em] text-ink-3">
        {filtered.length} of {dishes.length} {dishes.length === 1 ? "dish" : "dishes"}
      </div>
            ...
            <li key={d.id} className="overflow-hidden rounded-lg border border-rule bg-paper">
              ...
              <Link href={`/dishes/${d.id}`} className="flex gap-3 p-3 md:hidden">
                <div className="relative shrink-0">
                  ...
                  {isOwner && !d.public && (
                    <span
                      className="absolute top-1 right-1 inline-flex items-center rounded-pill bg-ink/80 px-1.5 py-[1px] text-[9px] font-semibold uppercase tracking-[0.08em] text-paper"
                ...
                {!isOwner && d.favorite && (
                  <div className="flex shrink-0 items-start">
                    <span className="grid h-8 w-8 place-items-center rounded-pill border border-rule bg-bg text-accent" aria-label="Favourite">
                ...
                <div className="p-4">
                  <div className="flex items-start gap-3">
                    ...
                    {isOwner ? (
                      ...
                      <span
                        className="grid h-9 w-9 shrink-0 place-items-center rounded-pill border border-rule bg-bg text-accent"
      ...
      {!dishes.length && (
        <div className="rounded-lg border border-dashed border-rule p-6 text-center text-ink-3">
          {isOwner ? "You haven't added any dishes yet." : "Nothing public here yet."}
        </div>
      )}
      {dishes.length > 0 && !filtered.length && (
        <div className="rounded-lg border border-dashed border-rule p-6 text-center text-ink-3">
